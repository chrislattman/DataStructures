...
/// @brief A lock-free SPSC (single producer, single consumer) queue (ring
/// buffer/FIFO) built on top of an array. SPSC queues are used in HFT
/// pipelines, audio/video processing, real-time control, and GPU/CPU
/// communication.
///
/// @tparam T data type
template <typename T> class MySPSCArrayQueue {
private:
  const size_t mask;
  T *buffer;
  // char pad0[64];                               // padding to avoid false
  // sharing on same 64 byte cache line
  std::atomic_size_t head;
  // char pad1[64 - sizeof(std::atomic_size_t)];  // padding to avoid false
  // sharing on same 64 byte cache line
  std::atomic_size_t tail;
  // char pad2[64 - sizeof(std::atomic_size_t)];  // padding to avoid false
  // sharing on same 64 byte cache line
...
public:
  ...
  MySPSCArrayQueue(size_t k)
      : mask((1 << k) - 1), buffer(new T[(1 << k)]), head(0), tail(0) {}

  /// @brief Frees dynamically allocated resources.
  virtual ~MySPSCArrayQueue() { delete[] buffer; }
  ...
  /// @brief Retrieves and removes the element at the front of this lock-free
  /// SPSC queue.
  ///
  /// @return element at the front of this queue, or NULL if queue is empty
  std::optional<T> poll() {
