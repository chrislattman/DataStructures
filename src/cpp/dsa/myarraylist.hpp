...
///
/// @tparam T data type
template <typename T> // replace <typename T> with <std::totally_ordered T> for
                      // a priority queue
...
private:
  ...
  /// @brief Doubles or halves the size of the internal array depending on size.
  ...
  void checkCapacity() {
    if (array_length == 0) {
      ...
      delete[] array;
      array = newArray;
    } else if (array_length > minArrLenThreshold &&
               array_size * 2 < array_length) {
...
public:
  /// @brief Constructs an array list instance with a specified initial
  /// capacity. Also known as the custom constructor.
  ///
  ...
  /// @brief Constructs an array list instance with a default initial capacity
  /// of 10. Also known as the default constructor.
  MyArrayList() : MyArrayList(defaultCapacity) {};

  /// @brief Constructs an array list instance with the copied contents of
  /// another array list. Also known as the copy constructor.
  ///
  ...
  /// @brief Reinitalizes assignee (left hand side) array list instance from
  /// another array list. Also known as the copy assignment operator.
  ///
  /// @param list other array list to deep copy from
  ...
  virtual ~MyArrayList() { delete[] array; }

  /// @brief Inserts an element at the specified index.
  ///
  /// @param index index to add element
  /// @param element element to add
  std::expected<void, std::string> add(unsigned int index,
                                       const T &element) override {
    try {
  ...
  ///
  /// @param element element to add
  void add(const T &element) override { add(array_size, element); }
  ...
  /// @brief Retrieves, but does not remove, an element from this list at the
  /// specified index.
  ///
  ...
  /// @brief Retrieves, but does not remove, an element from this list at the
  /// specified index. Can also be used to modify the array list.
  ///
  /// @param index index to retrieve element from
  /// @return element found
  ...
  /// @brief Returns the index of the first occurrence of an element found in
  /// this list.
  ///
  ...
  bool isEmpty() const override { return array_size == 0; }

  /// @brief Returns the index of the last occurrence of an element found in
  /// this list.
  ///
  /// @param element element to search for
  ...
  /// @brief Retrieves and removes an element from this list at a specified
  /// index.
  ///
  ...
  std::expected<T, std::string> set(unsigned int index,
                                    const T &element) override {
    try {
      checkIndex(index, array_size);
    } catch (const std::out_of_range &e) {
  ...
  ///
  /// @return size of list
  unsigned int size() const override { return array_size; }
  ...
  /// @brief Returns a string representation of this list, e.g. "[element1,
  /// element2, element3, ..., elementN]".
  ///
...
/// @brief Prints out the array list without needing to manually call
/// toString().
///
/// @tparam T data type
/// @param str ostream
