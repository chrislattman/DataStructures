...
///
/// @tparam T data type
template <typename T> class MyHashSet : public MySet<T> {
private:
  MyHashMap<T, int> *map;
...
public:
  /// @brief Constructs a hash set instance with the specified initial capacity
  /// and load factor.
  ///
  ...
  ///
  /// @param initialCapacity initial capacity of this hash set
  MyHashSet(unsigned int initialCapacity)
      : MyHashSet(initialCapacity, defaultLoadFactor) {}

  /// @brief Constructs a hash set instance with a default initial capacity of
  ...
  bool contains(const T &element) const { return map->containsKey(element); }
  ...
  bool remove(const T &element) { return map->remove(element) != T(NULL); }
  ...
  /// @brief Returns a string representation of this set, e.g. "[element1,
