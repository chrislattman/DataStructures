...
///
/// @tparam T data type
template <typename T> class MyLinkedList : public MyList<T> {
private:
...
public:
  ...
  /// @brief Constructs a linked list instance with the copied contents of
  /// another linked list.
  ///
  ...
  /// @brief Reinitalizes assignee (left hand side) linked list instance from
  /// another linked list.
  ///
  /// @param list other linked list to deep copy from
  ...
  ///
  /// @param element element to add
  void add(const T &element) { add(list_size, element); }
  ...
  bool isEmpty() const { return list_size == 0; }
...
/// @brief Prints out the linked list without needing to manually call
/// toString().
///
/// @tparam T data type
/// @param str ostream
