...
/// @brief A hash map data structure utilizing quadratic probing (open
/// addressing). Also called a hash table.
///
...
private:
  ...
  /// @brief Internal function used to add or modify a key-value pair in this
  /// map.
  ///
  ...
  V insert(const K &key, const V &newValue, const V &oldValue,
           bool addOnlyIfAbsent, bool addOnlyIfKeyExists) {
  ...
  /// @brief Increases the map size and rehashes the key-value pairs when load
...
public:
  /// @brief Constructs a hash map instance with the specified initial capacity
  /// and load factor.
  ///
  ...
  /// total slots in hash map) before rehashing occurs
  ...
  ///
  /// @param initialCapacity initial capacity of this hash map
  MyHashMap(unsigned int initialCapacity)
      : MyHashMap(initialCapacity, defaultLoadFactor) {}

  /// @brief Constructs a hash map instance with a default initial capacity of
  ...
  V get(const K &key) const { return getOrDefault(key, V(NULL)); }
  ...
  /// @brief Returns the value associated with the specified key, or a default
  /// value if they key was not found.
  ///
  /// @param key key to search for
  ...
  bool isEmpty() const { return map_size == 0; }
  ...
  /// @brief Inserts a key-value pair into this map. Null keys and NULL values
  /// are not permitted.
  ///
  /// @param key key to add
  ...
  /// @brief Inserts a key-value pair into this map only if key was not found.
  ...
  /// @return value associated with key prior to removal, or NULL if key was not
  /// found
  ...
  /// @brief Removes a key-value pair from this map only if the key's current
  ...
  /// @brief Replaces the value for a key with another value. Null values are
  /// not permitted.
  ///
  ...
  /// @brief Replaces the value for a key only if the key's current value
  /// matches the specified old value. Null values are not permitted.
  ///
  /// @param key key to modify value of
  ...
  ///
  /// @return size of map
  unsigned int size() const { return map_size; }

  /// @brief Returns a string representation of this map, e.g. "{key1=value1,
  /// key2=value2, key3=value3, ..., keyN=valueN}".
  ///
