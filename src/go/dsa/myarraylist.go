...
// NewDefault returns a new MyArrayList struct with a default initial capacity
// of 10.
func NewArrayListDefault[T comparable]() MyArrayList[T] {
	return NewArrayList[T](defaultCapacity)
}

// New returns a new MyArrayList struct with initial capacity initialCapacity.
func NewArrayList[T comparable](initialCapacity uint32) MyArrayList[T] {
	retval := new(MyArrayList[T])
	retval.array = make([]T, initialCapacity)
	retval.size = 0
	return *retval
}
